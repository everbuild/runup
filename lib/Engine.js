...
module.exports = class {
    ...
    resolveFlow(path, flowName) {
        ...
        if(!['dev', 'prd', 'check'].includes(flowName)) throw new Error(`Invalid flow scenario: ${flowName}`)
        const cat = this.model.assets.find(cat => cat[flowName] && (!cat.filter || this.match(path, cat.filter)))
        if(!cat) throw new Error(`No ${flowName} flow found for path "${path}"!`)
        return Promise.resolve(require(cat[flowName])).then(stop)
    }
