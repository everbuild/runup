...
module.exports = class extends Engine {
    ...
    get(path) {
        return Promise.resolve(this.getCached(path))
            .then(asset => asset && asset.errors.length === 0 ? asset : this.process(path, 'dev'))
    }
    ...
    process(path, flowName) {
        const promised = super.process(path, flowName)
        if(flowName === 'dev') this.cache.set(path, promised)
        return promised
    }
