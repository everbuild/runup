...
function normalize(model) {
    ...
    model.assets.forEach(cat => _.each(cat, (value, key) => {
        ...
        if(!_.isString(value)) throw new Error(`${key} flow of category matching ${cat.filter || 'all assets'} is not a string!`)
        cat[key] = value.split(/\s+/).map(file => require.resolve(p.join(model.flows, file)))
    }))
}
